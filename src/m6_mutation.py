...
def RETURN_replace_negatives_by_zeros(numbers):
    replaced_negative_list = [numbers[k] for k in range(len(numbers))]
    for k in range(len(replaced_negative_list)):
        if replaced_negative_list[k] < 0:
            replaced_negative_list[k] = 0
    return replaced_negative_list
    """
